...
int main(int argc, char **argv)
{
	...
	try {
		...
		args.registerArg(new bpp::ProgramArguments::ArgBool("serial", "Runs the serial implementation instead of the GPU."));
	...
	try {
		...
		try {
			bool serial = args.getArgBool("serial").getValue();
			bpp::Stopwatch stopwatch(true);
			if (serial)
				program = new SerialSimulator(graph.pointCount(), graph.getEdges(), graph.getLengths(), modelParameters);
			else 
